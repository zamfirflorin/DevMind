...
public class PlayerStatus {
	...
	public boolean shouldAttackOpponent(PlayerStatus opponent) {
		...
		if (this.getWeaponInHand().equals(opponent.getWeaponInHand())) {
			...
			if (this.probabilityToWin(this.getHealth(), this.getScore()) > opponent
					.probabilityToWin(opponent.getHealth(), opponent.getScore())) {
				return true;
			} else {
				return false;
			}
	...
	public void afiseazaStarea() {
		System.out.println("PlayerStatus [nickname=" + nickname + ", score=" + score + ", lives=" + lives + ", health="
				+ health + ", weaponInHand=" + weaponInHand + ", positionX=" + positionX + ", positionY=" + positionY
				+ "]");
	}
